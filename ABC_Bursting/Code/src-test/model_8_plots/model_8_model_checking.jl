...
number_unknown_params =4
n_samples = 500
n_tester = 500
tau = 500
tmax = tau*n_samples

fileending_2i = string("model_8_serum_genes_2i_data_")
fileending_serum = string("model_8_serum_genes_serum_data_")
binsize = 15
...
for name in genes
      ...
      data_file_2i=read_one_m8(path_to_file_2i,  3, n_samples,number_unknown_params)
      data_file_serum=read_one_m8(path_to_file_serum,  3, n_samples,number_unknown_params)
      ...
      for s in sample_rand_2i
          for i in 1:length(cum_posterior_2i)
                if s<cum_posterior_2i[i]
                  push!(winner_inds_2i, [data_file_2i[1][i], data_file_2i[2][i], data_file_2i[3][i], data_file_2i[4][i]])
                  break
                end
          end
      end
      ...
      for s in sample_rand_serum
          for i in 1:length(cum_posterior_serum)
                if s<cum_posterior_serum[i]
                  push!(winner_inds_serum, [data_file_serum[1][i], data_file_serum[2][i], data_file_serum[3][i],data_file_serum[4][i]])
                  break
                end
          end
      end
...
for i in 1:length(genes)
    ...
    for sim in 1:length(sampled_parameter_combinations_2i[i])
        one_gene_one_simulation_result_2i = generate_single_simulation_samples_m8(sampled_parameter_combinations_2i[i][sim], n_samples, data_scaling_RNAseq)
    ...
    for sim in 1:length(sampled_parameter_combinations_serum[i])
        one_gene_one_simulation_result_serum= generate_single_simulation_samples_m8(sampled_parameter_combinations_serum[i][sim], n_samples, data_scaling_RNAseq)
...
for gene in 1:length(genes)
    ...
    fig,ax = PyPlot.subplots()
      ...
      ax[:set_xlabel]("Product abunance",fontsize=16)
    ...
    name = string("/project/home17/er4517/project_3/gene_bursting/ABC_Bursting/plots/posterior_quantile/model_8_serum_genes_02/", genes[gene],fileending_2i,"_n_tester",string(n_tester),"_binsize",string(binsize),"1.pdf")
     # PyPlot.savefig(name)
     ...
     #   sns.tsplot(in_para_serum,color="colorblind", value="Frequency", time=np.linspace(all_genes_min_max_all[gene][1], all_genes_min_max_all[gene][2], binsize),  ci=95,ax=ax)
     #   sns.tsplot(in_para_2i,color="salmon", value="Frequency", time=np.linspace(all_genes_min_max_all[gene][1], all_genes_min_max_all[gene][2], binsize),  ci=95,ax=ax)
     #   sns.distplot(all_genes_experimental_results_2i[gene],norm_hist=true, bins=binsize, kde= false, hist=true,rug= false ,  color = "red", label = "experimental 2i",axlabel = "Product abundance",ax=ax)
     #   sns.distplot(all_genes_experimental_results_serum[gene],norm_hist=true, bins=binsize, kde= false, hist=true,rug= false ,  color = "blue", label = "experimental serum",axlabel = "Product abundance",ax=ax)
      ...
      #    sns.tsplot(in_para_serum,color="colorblind", value="Frequency", time=np.linspace(all_genes_min_max_all[gene][1], all_genes_min_max_all[gene][2], binsize),  ci=95,ax=ax)
      #    sns.tsplot(in_para_2i,color="salmon", value="Frequency", time=np.linspace(all_genes_min_max_all[gene][1], all_genes_min_max_all[gene][2], binsize),  ci=95,ax=ax)
      #    sns.distplot(all_genes_experimental_results_2i[gene],norm_hist=true, bins=25, kde= false, hist=true,rug= false ,  color = "grey", label = "experimental 2i")
      #    sns.distplot(all_genes_experimental_results_serum[gene],norm_hist=true, bins=25, kde= false, hist=true,rug= false ,  color = "grey", label = "experimental serum")
      #
      #    ax[:legend](fontsize=16)
      #
      # fig,ax = PyPlot.subplots()
      ...
      #     sns.tsplot(in_para_serum,color="colorblind", value="Frequency", time=np.linspace(all_genes_min_max_all[gene][1], all_genes_min_max_all[gene][2], binsize),  ci=95,ax=ax)
      #     sns.tsplot(in_para_2i,color="salmon", value="Frequency", time=np.linspace(all_genes_min_max_all[gene][1], all_genes_min_max_all[gene][2], binsize),  ci=95,ax=ax)
      #     sns.distplot(all_genes_experimental_results_2i[gene],norm_hist=true, bins=25, kde= false, hist=true,rug= false ,  color = "grey", label = "experimental 2i",axlabel = "Product abundance",ax=ax)
      #     sns.distplot(all_genes_experimental_results_serum[gene],norm_hist=true, bins=25, kde= false, hist=true,rug= false ,  color = "grey", label = "experimental serum",axlabel = "Product abundance",ax=ax)
      #     ax[:legend](fontsize=16)
