...
genes = ["ENSMUSG00000006398", "ENSMUSG00000027715", "ENSMUSG00000029177", "ENSMUSG00000029472", "ENSMUSG00000030867"]
#, "ENSMUSG00000041431"]
# genes = [ "ENSMUSG00000006398"]

#sample parameter combinations of approximated joint distr of ABC
xes=[]
winners=[]
for name in genes
  ...
  x=read_one_m9(pathtofile_lisi,  3, 100, 6)
  push!(xes,x)
  cum_posterior = Vector(100)
  #CHANGE TO 8!!!!!!!!!!!!!!!!!!
  cumsum!(cum_posterior, x[8])
  sample_rand = rand(10)
  winner_inds = []
  for s in sample_rand
      for i in 1:length(cum_posterior)
            if s<cum_posterior[i]
              push!(winner_inds, [x[1][i], x[2][i], x[3][i], x[4][i], x[5][i],x[6][i]])
              break
            end
      end
  end
...
for i in 1:length(genes)
    ...
    for sim in 1:length(winners[i])
        resb_sim = generate_single_simulation_samples_m9(winners[i][sim], 100, data_scaling_RNAseq)
        push!(resb,resb_sim)
    end
    push!(res_sim,resb)
    PyPlot.ion()
    ...
    name = string("/project/home17/er4517/project_3/gene_bursting/ABC_Bursting/plots/exper/model_9/", gene)
    PyPlot.savefig(name)
end
