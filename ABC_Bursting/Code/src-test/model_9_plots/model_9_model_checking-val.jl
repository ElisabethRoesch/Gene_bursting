...
newTrue=log10.([ 0.001, 0.0001, 0.04, 1000])
a=generate_single_simulation_samples_m9_no_deact(newTrue, 100, data_scaling_RNAseq)
PyPlot.ion()
fig,ax = PyPlot.subplots()
    sns.distplot(a,norm_hist=true, bins=binsize, kde= true, hist=false,rug= false ,  color = "blue", label = "experimental serum",axlabel = "Product abundance",ax=ax)
...
for i in 1:length(genes)
    ...
    for sim in 1:length(sampled_parameter_combinations_2i[i])
        one_gene_one_simulation_result_2i = c
