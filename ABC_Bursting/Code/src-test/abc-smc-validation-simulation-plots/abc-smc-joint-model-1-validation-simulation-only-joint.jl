...
for name in names
  ...
  path_to_file_2i = string("/project/home17/er4517/project_3/gene_bursting/ABC_Bursting/Data/ABC_output/RNAseq/validation_simulation/model_1/model1_",name,fileending_2i,".txt")
  ...
  path_to_plot = string("/project/home17/er4517/project_3/gene_bursting/ABC_Bursting/plots/validation-simulation/",name,fileending_2i,"_joint.png")
  ...
  # a_serum = pd.DataFrame(data= Dict( :c_act=>d_serum[:,1],:c_deact=>d_serum[:,2],:c_deg=>d_serum[:,3]))
      ...
      fig, ax = PyPlot.subplots(figsize=(15,15),ncols=3, nrows=1)
     ...
     # sns.distplot(c3_2i,bins=15, kde= true, hist=true,rug= false ,   color = "salmon", label = "lisi",axlabel = "degradation rate cdeg",ax=ax[3,3])
      sns.jointplot(x="c_act", y="c_deact", data=a_2i, kind="kde",color="lightpink", ax=ax[1,1])
      sns.jointplot(x="c_act", y="c_deg", data=a_2i, kind="kde", color="lightpink",ax=ax[1,2])
      sns.jointplot(x="c_deact", y="c_act", data=a_2i, kind="kde", color="lightpink",ax=ax[1,3])
